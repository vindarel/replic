...
#+BEGIN_SRC lisp
(replic.completion:*default-command-completion* #'my-function)
#+END_SRC
...
Lisp file, hence we need to remember the package they come from. These
lists and alist  are manipulated through an interface  hiding the data
structures (likely to change, too tedious  and error prone for a final
user) in =completion.lisp=.
